...
def compute_cell1(args):
    NoN, NoR, R, bw, dw, bm, dm, gw, gm, mu, NoT = args
    
    N0range = [round(10**x) for x in np.arange(2, 8, 6.0/NoN)]
...
NoNrange = range(2,41,2)
NoRrange = range(10,121,5) 
NoT = 200
...
for R in [10**6, 10**5]:
    ...
    for NoN in NoNrange:
        print(NoN)
        args_list = [
            (NoN, NoR, R, bw, dw, bm, dm, gw, gm, mu, NoT)
            for NoR in NoRrange
        ]
        with ProcessPoolExecutor(max_workers=14) as executor:
            row = list(executor.map(compute_cell1, args_list))
        detection_probability.append(row)
    data_NoNvsNoR[R] = detection_probability

with open("data_NoNvsNoR.pkl", "wb") as f:
    pickle.dump(data_NoNvsNoR, f) 
    
def compute_cell_start2(args):
    start, NoR, R, bw, dw, bm, dm, gw, gm, mu, NoT = args

    N0range = [round(10**x) for x in np.arange(start, start+4.01, 0.5)]
    Rrange = [R for _ in N0range]
    plist = power_analysis_alternate(
        bw, dw, bm, dm, Rrange, gw, gm, N0range, mu, NoR, NoT
    )
...
with ProcessPoolExecutor(max_workers=16) as executor:
    for R in [10**6, 10**5]:
        ...
        for start in Startrange:
            args_list = [
                (start, NoR, R, bw, dw, bm, dm, gw, gm, mu, NoT)
                for NoR in NoRrange
            ]

            row = list(executor.map(compute_cell_start2, args_list))
            detection_probability.append(row)

        data_StartvsNoR[R] = detection_probability
    
with open("data_StartvsNoR.pkl", "wb") as f:
    pickle.dump(data_StartvsNoR, f) 
    
 
    
def compute_cell_width3(args):
    start, width, R, bw, dw, bm, dm, gw, gm, mu, NoR, NoT = args

    N0range = [round(10**x) for x in np.arange(start, start + width, width/8.0)]
    Rrange = [R for _ in N0range]
...
with ProcessPoolExecutor(max_workers=16) as executor:
    for R in [10**6, 10**5]:
        ...
        for start in Startrange:
            args_list = [
                (start, width, R, bw, dw, bm, dm, gw, gm, mu, NoR, NoT)
                for width in Widthrange
            ]

            row = list(executor.map(compute_cell_width3, args_list))
            detection_probability.append(row)

        data_StartvsWidth[R] = detection_probability

with open("data_StartvsWidth.pkl", "wb") as f:
    pickle.dump(data_StartvsWidth, f)